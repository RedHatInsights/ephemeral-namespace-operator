...
var _ = Describe("Reservation controller basic reservation", func() {
	Context("When creating a Reservation Resource", func() {
		...
		It("Should ensure two namespaces are ready after all reservations are reconciled", func() {
			...
			r1 := helpers.NewReservation(resName1, "30m", "test-user-03", "minimal", "default")
			r2 := helpers.NewReservation(resName2, "30m", "test-user-04", "minimal", "default")
			r3 := helpers.NewReservation(resName3, "30m", "test-user-05", "minimal", "default")
...
var _ = Describe("Handle reservation without defined pool type", func() {
	Context("When creating a Reservation Resource without specifying the pool", func() {
		It("Should be able to set the pool to the 'default' pool", func() {
			ctx := context.Background()
			resName := "test-res-06"
			reservation := helpers.NewReservation(resName, "30s", "test-user-06", "", "default")
...
var _ = Describe("Handle waiting reservations", func() {
	Context("When there are more reservations than ready namespaces", func() {
		It("Should create an excess of reservations to force some into a waiting state", func() {
			...
			res1 := helpers.NewReservation(resName1, "30m", "test-user-07", "minimal", "default")
			res2 := helpers.NewReservation(resName2, "30m", "test-user-08", "minimal", "default")
			res3 := helpers.NewReservation(resName3, "30m", "test-user-09", "minimal", "default")
			res4 := helpers.NewReservation(resName4, "30m", "test-user-10", "minimal", "default")
			res5 := helpers.NewReservation(resName5, "30m", "test-user-11", "minimal", "default")
